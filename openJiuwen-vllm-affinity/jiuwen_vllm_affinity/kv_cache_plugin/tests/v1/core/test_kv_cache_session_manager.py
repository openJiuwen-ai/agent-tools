...
from jiuwen_vllm_affinity.kv_cache_plugin.v1.core.kv_cache_session_manager import KvCacheSessionManager

def test_kv_cache_session_manager_basic():
    block_count = 3
    blocks: list[KVCacheBlock] = [
        KVCacheBlock(idx) for idx in range(block_count)
    ]
    sessions = ["s1", "s2"]
...
def test_kv_cache_session_manager():
    ...
    assert released[0].block_id == 0
    assert released[0].block_id == 1
    released = session_mgr.release_blocks(blocks, sessions[0])
