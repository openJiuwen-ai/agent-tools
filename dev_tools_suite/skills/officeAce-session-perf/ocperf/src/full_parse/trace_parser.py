...
def _parse_ts_text(text: str) -> datetime | None:
    text = (text or "").strip()
...
def parse_trace_message(
    ...
    *,
    ...
    if "[LLM_IO_TRACE]" not in message:
        return None
    m = _HDR_RE.search(message)
    if not m:
        return None
    bp, rseq, body = _extract_body_segment(message)
    return TraceRecord(
        ts=ts,
        event=m.group("event"),
...
def parse_trace_line(line: str, line_no: int) -> TraceRecord | None:
    ...
    if ts is None:
        return None
    return parse_trace_message(line, ts, line_no)


def parse_json_log_object(
    obj: dict[str, Any],
    line_no: int,
    *,
    source_file: str = "",
    ...
    ts = _parse_ts_text(str(obj.get("timestamp") or "")) or parse_ts(message)
    if ts is None:
        return None
    return parse_trace_message(
        message,
        ts,
        line_no,
        source_file=source_file,
...
def assemble_records(raw_records: list[TraceRecord]) -> list[TraceRecord]:
    ...
    return sorted(out, key=lambda r: (r.ts, r.file_order, r.raw_line_no))
...
def extract_tokens_from_body(body: str) -> dict[str, int | None]:
    ...
    for name in cache_keys:
        m = re.search(rf"{name}[=:]?\s*(\d+)", body)
        if m:
            out["cache_tokens"] = (out["cache_tokens"] or 0) + int(m.group(1))
    if body.strip().startswith("{"):
        try:
            ...
            if isinstance(um, dict):
                ...
                cache = extract_cache_tokens(um)
                if cache:
                    out["cache_tokens"] = cache
