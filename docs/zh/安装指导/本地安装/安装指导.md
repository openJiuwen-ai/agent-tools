...
- **接口代理**：开发环境下，浏览器请求 **`/api/v1`** 会由 Vite 转发到 **`VITE_API_PROXY_TARGET`**；未配置时默认为 `http://localhost:8000`。请与 **marketplace 实际监听地址**一致。例如根目录 `.env` 中 `STORE_PORT=8100` 时，建议在仓库根 `.env` 增加：

  ```env
  VITE_API_PROXY_TARGET=http://127.0.0.1:8100
  ```
...
- **前端页面无法加载插件列表 / 接口连错端口**：确认 marketplace 已启动，且仓库根目录 `.env` 中 **`VITE_API_PROXY_TARGET`** 与 **`STORE_HOST` / `STORE_PORT`** 一致（见第 6 节）。
