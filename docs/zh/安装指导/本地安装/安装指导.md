...
(3). 在 `.env` 中配置（示例）：
   ...
   - `MARKET_STORAGE_PUBLIC_URL`：用于生成 `icon_uri` 等浏览器可访问地址，一般为 `http(s)://主机:端口/桶名` 或对外域名
...
(3). 在 `.env` 中配置（示例）：
   ...
   - `MARKET_BUCKET_NAME` 与 OBS 桶名一致
   - `MARKET_STORAGE_PUBLIC_URL`：对外可访问的基地址（可为桶访问域名或绑定的 CDN 域名）
