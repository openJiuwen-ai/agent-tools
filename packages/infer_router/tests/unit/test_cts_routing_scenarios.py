...
class TestCTSWorkerPairSelection:
    ...
    @staticmethod
    def test_sequential_dispatch_q1_to_q10_twice(cts_algorithm, two_groups_workers, kv_cache_manager):
        ...
        # 记录测试信息
        logger.info("\n" + "=" * 80)
        logger.info("测试用例1：顺序分发 q1-q10 两遍")
        logger.info("=" * 80)
        logger.info(
        ...
        logger.info(f"初始负载: {initial_loads}")
        ...
        for round_num in range(2):
            ...
            for req_id, request, overlap, token_count in requests:
                ...
                try:
                    prefill_worker, decode_worker = cts_algorithm.select_worker_pair(workers, groups, request)

                    # 计算分数以供验证
                    prefill_workers = [
                        w for w in workers.values() if w.worker_type in (WorkerType.PREFILL, WorkerType.COMBINED)
                    ]
                    prefill_score = cts_algorithm.calculate_score(prefill_worker, prefill_workers, request)
                    decode_workers = [
                        w for w in workers.values() if w.worker_type in (WorkerType.DECODE, WorkerType.COMBINED)
                    ]
                    decode_score = cts_algorithm.calculate_score(decode_worker, decode_workers, request)

                    logger.info(
                        f"{req_id:<8} {token_count:<8} {overlap:.0%}     "
                        f"{prefill_worker.worker_id:<20} {decode_worker.worker_id:<20} "
                        f"{prefill_score:< 15.3f} {decode_score:< 15.3f} "
                        f"{prefill_worker.current_load:< 12} {decode_worker.current_load:< 12}"
                    )

                    # 验证：分数应该是正数
                    assert prefill_score >= 0, f"{prefill_worker.worker_id} 的分数 {prefill_score} 不应为负"
                    assert decode_score >= 0, f"{decode_worker.worker_id} 的分数 {decode_score} 不应为负"

                    # 验证：选中的 worker 应该在同一个 group
                    selected_group = None
                    for group_name, worker_ids in groups.items():
                        if prefill_worker.worker_id in worker_ids and decode_worker.worker_id in worker_ids:
                            selected_group = group_name
                            break
                    assert selected_group is not None, "选中的 prefill 和 decode worker 应该在同一个 group"
        ...
        logger.info("\n" + "=" * 80)
        logger.info("测试用例1 完成")
        logger.info("=" * 80)

    @staticmethod
    def test_reverse_dispatch_q10_to_q1_twice(cts_algorithm, two_groups_workers, kv_cache_manager):
        ...
        # 记录测试信息
        logger.info("\n" + "=" * 100)
        logger.info("测试用例2：逆序分发 q10-q1 两遍")
        logger.info("=" * 100)
        logger.info(
            f"{'请求ID':<8} {'Token数':<8} {'重叠度':<10} "
            f"{'选中的Prefill':<20} {'选中的Decode':<20} "
        ...
        logger.info("\n" + "=" * 80)
        logger.info("测试用例2 完成")
        logger.info("=" * 80)

    @staticmethod
    def test_shuffled_dispatch_with_duplicates(cts_algorithm, two_groups_workers, kv_cache_manager):
        ...
        # 记录测试信息
        logger.info("\n" + "=" * 110)
        logger.info("测试用例3：打乱顺序分发（每请求两份）")
        logger.info("=" * 110)
        logger.info(
            f"{'分发ID':<12} {'原始请求':<8} {'Token数':<8} {'重叠度':<10} {'选中的Prefill':<20} "
            f"{'选中的Decode':<20} {'Prefill分数':<15} {'Decode分数':<15} {'Prefill负载':<12} {'Decode负载':<12}"
        )
        logger.info("-" * 140)
        ...
        for dispatch_id, (req_id, request, overlap, token_count) in enumerate(all_requests, 1):
            ...
            try:
                ...
                logger.info(
                    f"{dispatch_id:<12} {req_id:<8} {token_count:<8} {overlap:.0%}     "
                    f"{prefill_worker.worker_id:<20} {decode_worker.worker_id:<20} "
                    f"{prefill_score:< 15.3f} {decode_score:< 15.3f} "
                    f"{prefill_worker.current_load:< 12} {decode_worker.current_load:< 12}"
                )
                ...
                logger.error(f"{req_id} (round {dispatch_id}) 分发失败: {e}")
                raise
        ...
        for req_id, history in request_selection_history.items():
            if len(history) >= 2:
                ...
                logger.info(f"{req_id}: Prefill 选择 {len(prefill_choices)} 种, Decode 选择 {len(decode_choices)} 种")

        logger.info("\n" + "=" * 80)
        logger.info("测试用例3 完成")
        logger.info("=" * 80)
...
class TestCTSLoadBalancing:
    ...
    def test_load_balance_with_different_worker_loads(cts_algorithm, kv_cache_manager):
        ...
        logger.info(
            ...
            f"({selection_count['high-load'] / num_iterations * 100:.1f}%)"
        )
        logger.info(
            f"  low-load 被选中: {selection_count['low-load']} 次 "
            f"({selection_count['low-load'] / num_iterations * 100:.1f}%)"
        )
    ...
    def test_all_scores_are_non_negative(cts_algorithm, kv_cache_manager):
        ...
        for req_id, request, _overlap, token_count in requests:
            ...
            for worker in workers.values():
                ...
                # 验证分数非负
                assert score >= 0, f"分数不应为负: req={req_id}, worker={worker.worker_id}, score={score}"
        ...
        logger.info(f"  最小值: {min_score:.3f}")
        logger.info(f"  最大值: {max_score:.3f}")
        logger.info(f"  平均值: {avg_score:.3f}")
