...
def test_prefill_overlap():
    ...
    logging.info("=== 测试Prefill Worker Overlap ===")

    # 初始化组件
    event_manager = KVEventManager()
    event_generator = KVEventGenerator()
    kvcache_manager = KVCacheManager(
        event_manager=event_manager, event_generator=event_generator, enable_radix_tree=True
    )
    ...
    for worker_id, worker_type in workers:
        ...
        logging.info(f"注册工作器: {worker_id} ({worker_type})")
    ...
    logging.info(f"Token数量: {len(route_hint.token_ids)}")
    logging.info(f"Block大小: {kvcache_manager.block_size}")
    ...
    logging.info(f"Overlap scores: {overlap_scores2}")
    ...
    decode_workers = ["worker-2-decode", "worker-3-decode", "worker-5-decode", "worker-6-decode"]

    logging.info("=== 结果分析 ===")
    for worker in prefill_workers_with_cache:
        score = overlap_scores2.get(worker, 0)
        if score > 0:
            logging.info(f"✓ {worker}: overlap={score} (预期)")
        else:
            logging.info(f"✗ {worker}: overlap={score} (应该大于0)")

    for worker in prefill_workers_without_cache:
        score = overlap_scores2.get(worker, 0)
        if score == 0:
            logging.info(f"✓ {worker}: overlap={score} (预期 - 没有存储缓存)")
        else:
            logging.info(f"✗ {worker}: overlap={score} (应该为0)")
