...
class OpenAIServingChatEx(OpenAIServingChat):
    def _invalid_auto_tool_choice(
        self,
        request: ChatCompletionRequest | ReleaseKvCacheRequest,
        tokenizer,
        tool_parser,
    ) -> bool:
        """Return True if 'auto' tool choice lacks required server configuration."""
        ...
        if isinstance(tokenizer, MistralTokenizer):
            return False
        auto_tools_ready = self.enable_auto_tools and tool_parser is not None
        return not auto_tools_ready
    ...
    async def create_chat_completion(
        ...
        try:
            ...
            if self._invalid_auto_tool_choice(request, tokenizer, tool_parser):
        ...
        try:
            for i, engine_prompt in enumerate(engine_prompts):
                ...
                if isinstance(sampling_params, BeamSearchParams):
                    ...
                    sharing_cache_salt = (
                        request.cache_salt
                        if hasattr(request, "cache_sharing") and request.cache_sharing
                        else None
                    )
    ...
    async def release_kv_cache(
        ...
        try:
            ...
            if (
                after_tool_dicts is not None
                and request.tools_released_index is not None
            ):
                after_tool_dicts = tool_dicts[: request.tools_released_index]
            tool_changed = after_tool_dicts is not None and len(tool_dicts) != len(
                after_tool_dicts
            )
            ...
            if not self.use_harmony:
                ...
                if error_check_ret is not None:
                    return error_check_ret
                before_request_prompts, before_engine_prompts = (
        ...
        if len(before_engine_prompts) != 1 or len(after_engine_prompts) != 1:
            error_msg = (f"engine prompts should be 1, while older is {len(before_engine_prompts)} "
        ...
        for i, engine_prompt in enumerate(before_engine_prompts):
            if (
                ...
                and prompt_token_ids_key in engine_prompt
            ):
        ...
        released_num = await self.engine_client.release_kv_cache(
            request.cache_salt, request_params
        )
        return ReleaseKvCacheResponse(
...
def register_openai_serving():
    ...
    OpenAIServingChat.create_chat_completion = (
        OpenAIServingChatEx.create_chat_completion
    )
